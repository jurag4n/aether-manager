...
object RuntimeUiText {
    ...
    fun phoneRequired(context: Context) = if (isId(context)) "Nomor WhatsApp wajib diisi" else "WhatsApp number is required"
    fun phoneInvalid(context: Context) = if (isId(context)) "Format nomor tidak valid. Contoh: 08123456789 atau +14155552671" else "Invalid number format. Example: 08123456789 or +14155552671"
    fun phoneTooShort(context: Context) = if (isId(context)) "Nomor terlalu pendek" else "Number is too short"
    fun phoneTooLong(context: Context) = if (isId(context)) "Nomor terlalu panjang" else "Number is too long"
...
val AppStrings.licenseIntlCreateSubtitle: String get() = "Enter your buyer details. International users can use WhatsApp numbers with country code."
val AppStrings.licenseLocalCreateSubtitle: String get() = if (this === StringsId) "Isi data pembeli untuk membuat order real. Nomor luar Indonesia juga didukung." else "Enter buyer details to create a real order. International numbers are supported."
...
val AppStrings.licenseLocalPaymentHint: String get() = if (this === StringsId) "Untuk Indonesia gunakan DANA/GoPay. Untuk luar Indonesia gunakan PayPal dan nomor +kode negara." else "Use DANA/GoPay for Indonesia. International buyers can use PayPal and a country-code phone number."
...
val AppStrings.licenseIntlPhoneHint: String get() = "WhatsApp number, example +14155552671"
